...
elseif ($endpoint === 'profile') {
    ...
    elseif ($method === 'PUT') {
        ...
        try {
            ...
            // Handle emergency contact update if any emergency contact field provided
            $ecFirstName = $input['emergency_contact_first_name'] ?? '';
            $ecLastName = $input['emergency_contact_last_name'] ?? '';
            $ecRelationship = $input['emergency_contact_relationship'] ?? '';
            
            if (!empty($emergencyContact) || !empty($ecFirstName) || !empty($ecLastName) || !empty($ecRelationship)) {
                ...
                if ($existingEcId) {
                    ...
                    $ecStmt = $mysqli->prepare("UPDATE emergency_contact SET ec_first_name = ?, ec_last_name = ?, ec_phone = ?, relationship = ? WHERE emergency_contact_id = ?");
                    $ecStmt->bind_param('ssssi', $ecFirstName, $ecLastName, $emergencyContact, $ecRelationship, $existingEcId);
                    $ecStmt->execute();
                } else {
                    // Create new emergency contact record
                    $ecStmt = $mysqli->prepare("INSERT INTO emergency_contact (patient_id, ec_first_name, ec_last_name, ec_phone, relationship) VALUES (?, ?, ?, ?, ?)");
                    $ecStmt->bind_param('issss', $patient_id, $ecFirstName, $ecLastName, $emergencyContact, $ecRelationship);
                    $ecStmt->execute();
